...
df <- read.csv("2019_Multifamily_ECAD.csv", stringsAsFactors = F)

#Websites
#Data: https://data.austintexas.gov/Utilities-and-City-Services/2019-Multifamily-ECAD/feiy-7jhe
#effect_plot(): https://cran.r-project.org/web/packages/jtools/vignettes/effect_plot.html
...
## Exporation and Cleaning

First, read in the dataset. 

```{r}
#Download from my GitHub folder.
df <- read.csv("linear_ecad_austin.csv", stringsAsFactors = F)
```
...
Sixth, check for influential points.

```{r}
ols_plot_cooksd_chart(model) 
```

Examine influential rows individually... 
...
...and remove rows as needed.

Seventh, and last, check the model's performance with test data. Compare RMSEs.
