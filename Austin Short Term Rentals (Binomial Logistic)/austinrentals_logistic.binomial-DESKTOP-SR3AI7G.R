...
#Fees per deficiency.
df %>%
  filter(DEFICIENCYTEXT == "Incomplete Registration Violation") %>%
  summarise(mean(OUTSTANDINGFEE))
df %>%
  filter(DEFICIENCYTEXT == "Licensing Violation") %>%
  summarise(mean(OUTSTANDINGFEE))
df %>%
  filter(DEFICIENCYTEXT == "Occupancy Limit Violation") %>%
  summarise(mean(OUTSTANDINGFEE))

#Likelihood of penalty if you...
#Get to court ASAP...
train %>%
  filter(PendingPeriod == "Pay it ASAP") %>%
  summarize(percent(mean(fitted)))
#Drag it out...
train %>%
  filter(PendingPeriod == "Delay Payment") %>%
  summarize(percent(mean(fitted)))
#Have a low fee...
train %>%
  filter(FeeLevel == "Low") %>%
